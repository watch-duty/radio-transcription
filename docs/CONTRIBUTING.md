...
3. Mock Audio server (simulates all the supported audio streams for testing e.g. Icecast and API polling). See [docs/local-dev-mock-audio.md](docs/local-dev-mock-audio.md) for instructions on adding test audio files.
...
1. Start the entire local environment:
   ...
   ```
   > [!TIP]
   > Use `mise run dev:add-audio` to quickly mock incoming audio files for specific feeds. See [docs/local-dev-mock-audio.md](docs/local-dev-mock-audio.md) for usage instructions.
