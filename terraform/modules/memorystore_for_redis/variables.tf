...
variable "transit_encryption_mode" {
  ...
  validation {
      condition     = contains(["DISABLED", "SERVER_AUTHENTICATION"], var.connect_mode)
      error_message = "transit_encryption_mode must be one of: DISABLED, SERVER_AUTHENTICATION."
    }
}

variable "prevent_destroy" {
    description = "Whether to prevent accidental destruction of the resource and cause Terraform to reject with an error any plan that would destroy the resource."
    type        = bool
    default     = true
}