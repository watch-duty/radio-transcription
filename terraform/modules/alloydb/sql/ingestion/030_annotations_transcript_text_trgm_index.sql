...
-- Set a larger gin_pending_list_limit to allow autovacuum to merge pending entries in the background,
...
WITH (gin_pending_list_limit = '128MB')
WHERE type = 'TRANSCRIPT'::annotation_type;

-- Update existing index if it was created without the storage parameter.
ALTER INDEX IF EXISTS idx_annotations_transcript_text_trgm SET (gin_pending_list_limit = '128MB');