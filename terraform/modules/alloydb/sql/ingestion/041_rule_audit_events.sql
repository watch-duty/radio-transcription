...
CREATE TABLE IF NOT EXISTS rule_audit_events (
    ...
    CONSTRAINT rule_audit_events_action_check CHECK (
        action IN (
            'rule.created',
            'rule.updated',
            'rule.deleted'
    ...
    CONSTRAINT rule_audit_events_actor_id_check CHECK (
        char_length(actor_id) > 0
        AND char_length(actor_id) <= 512
        AND actor_id !~ '[[:space:]]'
    ),
    CONSTRAINT rule_audit_events_revision_positive CHECK (rule_revision > 0),
    CONSTRAINT rule_audit_events_rule_revision_unique UNIQUE (rule_id, rule_revision),
    CONSTRAINT rule_audit_events_json_object_shape CHECK (
