-- Fail closed before CREATE INDEX CONCURRENTLY IF NOT EXISTS can silently
-- accept a same-name wrong or unhealthy relation. Discovery deliberately
-- starts from the public relation name rather than indexes on the target table.
DO $preflight$
DECLARE
...
BEGIN
    ...
    IF named_relation_count = 0 THEN
        RETURN;
    END IF;
    ...
    IF named_relation_count <> 1 THEN
        RAISE EXCEPTION USING
            MESSAGE =
                'Unexpected membership-index relation count in public',
