-- Exercise durable ingestion Lease behavior without depending on PostgreSQL's
-- internal catalog representation. All fixtures are rolled back.
BEGIN;
...
BEGIN
    ...
    VALUES (source_type_value, 'ingestion-lease-behavior-check');
    ...
    BEGIN
        INSERT INTO public.ingestion_leases (
            ...
            source_type_value,
            'ingestion-lease-ownerless-active-check',
            'active'
        );
    ...
    BEGIN
        UPDATE public.ingestion_leases
           SET lease_key = 'ingestion-lease-rekeyed'
         WHERE source_type = source_type_value
           AND lease_key = 'ingestion-lease-behavior-check';
