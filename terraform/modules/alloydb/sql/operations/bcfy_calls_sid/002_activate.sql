...
\if :{?reviewed_sid_count}
\else
\echo 'reviewed_sid_count psql variable is required'
\quit 3
\endif
\if :{?reviewed_manifest_digest}
\else
\echo 'reviewed_manifest_digest psql variable is required'
\quit 3
\endif
...
BEGIN
    ...
    IF review.process_absence_confirmed <> 'CONFIRMED'
       OR review.reviewed_sid_count <> 19
       OR review.reviewed_manifest_digest !~ '^[0-9a-f]{64}$' THEN
        RAISE EXCEPTION
    ...
    SELECT
      ...
      FROM phase7_expected_membership;
    IF expected_sid_count <> review.reviewed_sid_count
       OR expected_feed_count <> 154
       OR expected_digest IS DISTINCT FROM review.reviewed_manifest_digest THEN
        RAISE EXCEPTION
...
BEGIN
    ...
    IF transition_count <> 19 THEN
        RAISE EXCEPTION
            'activation changed % Leases, expected 19', transition_count;
    END IF;
