...
-- PostgreSQL does not permit the temporary assertion tables below in a
-- READ ONLY transaction.  This script makes no persistent-table mutations;
-- repeatable read keeps every assertion and report on one snapshot.
...
BEGIN
    ...
    IF review.reviewed_eligible_feed_count <= 0
       ...
       OR review.reviewed_eligible_manifest_digest !~ '^[0-9a-f]{64}$' THEN
        RAISE EXCEPTION 'verification review input is invalid';
    END IF;
    SELECT COUNT(*)
