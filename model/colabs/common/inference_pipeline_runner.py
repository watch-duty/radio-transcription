...
def run_huggingface_inference_pipeline(
    ...
    results_list = []
    logger.info(f"--- Starting parallel Hugging Face ASR pipeline for {len(manifest_data)} entries ---")
    ...
    for i in range(0, len(manifest_data), batch_size):
        ...
        for entry in batch:
            ...
            try:
                ...
                logger.error(f"Failed to download or preprocess {audio_blob_path}: {e}")
                continue
        ...
        try:
            ...
            logger.info(f"Processing parallel GPU batch {i//batch_size + 1}...")
            with torch.no_grad():
