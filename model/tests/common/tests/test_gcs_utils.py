    ...
    def test_returns_path_within_scratch_dir(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as scratch_dir:
            with unittest.mock.patch(
                ...
                result = gcs_utils_lib.download_to_scratch(
                    mock_client, "gs://bucket/path/audio.flac", scratch_dir
                )
            self.assertEqual(
                pathlib.Path(result).parent, pathlib.Path(scratch_dir)
            )
    ...
    def test_two_calls_yield_distinct_paths(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as scratch_dir:
            with unittest.mock.patch(
                ...
                result1 = gcs_utils_lib.download_to_scratch(
                    mock_client, "gs://bucket/path/audio.flac", scratch_dir
                )
                result2 = gcs_utils_lib.download_to_scratch(
                    mock_client, "gs://bucket/path/audio.flac", scratch_dir
                )
    ...
    def test_non_gs_uri_raises_value_error(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as scratch_dir:
            with self.assertRaises(ValueError):
                gcs_utils_lib.download_to_scratch(
...
class TestGcsObjectHelpers(unittest.TestCase):
    def test_gcs_uri_exists_returns_object_existence(self) -> None:
        ...
        self.assertTrue(
            gcs_utils_lib.gcs_uri_exists(storage, "gs://bucket/path/file.json")
        )
        self.assertFalse(
            gcs_utils_lib.gcs_uri_exists(
                storage, "gs://bucket/path/missing.json"
            )
        )
    ...
    def test_gcs_prefix_has_any_blob_checks_prefix(self) -> None:
        ...
        self.assertTrue(
            gcs_utils_lib.gcs_prefix_has_any_blob(
                storage, "gs://bucket/runs/a/"
            )
        )
        self.assertFalse(
            gcs_utils_lib.gcs_prefix_has_any_blob(
                storage, "gs://bucket/runs/b/"
            )
        )
    ...
    def test_download_json_text_requires_json_object(self) -> None:
        ...
        self.assertEqual(
            gcs_utils_lib.download_json_text(
                storage, "gs://bucket/config.json"
            ),
            {"status": "ok"},
        )
        with self.assertRaisesRegex(TypeError, "Expected JSON object"):
            gcs_utils_lib.download_json_text(storage, "gs://bucket/list.json")
    ...
    def test_upload_and_download_local_text_artifacts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_s:
            ...
            gcs_utils_lib.upload_local_file(
                storage, source, "gs://bucket/path/source.txt"
            )
            gcs_utils_lib.download_gcs_uri(
                storage, "gs://bucket/path/source.txt", dest
            )
            gcs_utils_lib.upload_text(
                storage, "inline", "gs://bucket/path/inline.txt"
            )
    ...
    def test_download_gcs_directory(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_s:
            ...
            gcs_utils_lib.download_gcs_directory(
                storage, "gs://bucket/dir", tmp_dir
            )
