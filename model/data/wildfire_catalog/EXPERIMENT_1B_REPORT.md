...
7. **Fleet-sizing translation.** We convert the coefficients into worker and VM counts for the 12,000-feed production target under the k=2-per-VM configuration demonstrated in §6.4 (n=1 in VMs, days, and machine types). We explicitly bound the scope: k=4+ multi-worker scaling, gunicorn-vs-multi-container architecture, and CFS-throttling-under-CPU-limits are not measured.
...
**Saturation-robustness refit (five-step fit excluding step 6).** Step 6 sits at 108.3% mean CPU, past the single-core ceiling; including it in a linear fit risks pulling the slope if the true curve bends up past saturation. Refitting only the five pre-saturation steps (active means 99.0, 248.7, 498.1, 745.6, 993.2) gives **CPU(%) = 0.0722 × active_feeds + 5.26** (+4.8% slope vs the 6-step active-feed fit of 0.0697). At the 1,000-feed recommended operating point, the 5-step fit predicts 77.5% single-core (the 6-step fit predicts 76.0%); observed at step 5 is 77.44%. Both fits are close to observation at the recommended operating point, and the 4.8% slope delta is well within the single-run population-uncertainty window (§7 item 1). We retain the 6-step target-based fit for headline claims because it is the most data-inclusive model; we report the 5-step fit as a robustness check confirming that the 1,000-feeds/worker recommendation is not an artifact of extrapolation from saturation-regime data.
...
The `loop_latency_ms = 0.02 ms` value reports the loop-scheduling latency measured *after* the freeze ended, not during it. **This is a structural limitation of an in-process event-loop monitor**: the monitor task is itself an asyncio coroutine on the same loop, so during a freeze the monitor is also frozen; the next value it emits reflects post-freeze responsiveness, not mid-freeze state. The 0.02 ms reading confirms the loop resumed cleanly, but it does *not* distinguish (a) pure starvation (the loop thread received no CPU during the freeze) from (b) a slow blocking operation on the loop thread (e.g., a `posix_spawn` system call whose kernel path blocked the caller for many seconds). Both mechanisms produce the same in-process signature (large `drift_ms`, small post-freeze `loop_latency_ms`). Definitive separation requires off-CPU profiling at the kernel level (eBPF `runqlat` / `offcputime`, deferred per §7 item 16).

The freeze coincides with cgroup CPU saturation on both containers simultaneously:
...
Both containers exceeded 100% single-core simultaneously during the t=0→30s activation window while spawning ~400 `ffmpeg -c copy` subprocesses each. This attributes the 9.7-s and 14–15-s stall *class* to **concurrent mass `posix_spawn` subprocess creation from multiple workers on a 4-vCPU VM**. The specific loop-level mechanism — starvation (the loop thread was never scheduled during the freeze) vs. a blocking-syscall hold (a `posix_spawn` call blocked the loop thread for the full freeze) — cannot be distinguished by the in-process monitor and requires off-CPU profiling to resolve (§7 item 16). Both mechanisms point to the same remediation direction. Mitigation: stagger worker activation (seconds-apart per-worker feed claims) rather than simultaneous mass claim, or cap concurrent `posix_spawn` invocations in each worker with a semaphore.
...
| Slow callback or blocking syscall on loop thread | `loop_latency_ms ≈ drift_ms` at the stall — but only measurable if the monitor is not itself frozen | `loop_latency_ms = 0.02 ms` measured *after* freeze end; monitor frozen during freeze | **Not distinguishable from starvation** with in-process monitor; requires eBPF off-CPU profiling |
...
Of the five alternatives, one is ruled out by positive evidence (I/O wait: zero GCS failures, nominal lease-ack latencies); one is structurally untestable in this configuration (CFS throttling: no `cpu.max` set); one (slow callback or blocking syscall on the loop thread) is **not distinguishable from pure starvation by the in-process monitor** and requires off-CPU profiling to resolve; and two (`getaddrinfo`, PgBouncer) are unmeasured and cannot be definitively excluded without the specific instrumentation deferred to Phase 2 (eBPF `runqlat` + `SHOW POOLS` snapshot during the burst). The attribution therefore is: the stall *class* is caused by concurrent mass subprocess creation, as evidenced by the cgroup CPU spike to >100% on both containers during the exact burst window; whether the specific per-thread mechanism is starvation, a slow `posix_spawn` syscall, or a `getaddrinfo` thread-pool stall on the same critical path is not resolved here.
...
The 2-workers-per-VM column is **demonstrated at k=2 in a single run** (§6.4) at ~1,000 feeds on one n2-standard-4 VM, with per-container steady-state CPU of 40.0% and 45.2% respectively (sum 85.2%, vs prediction 85.7%, residual −0.5%). "Demonstrated" rather than "validated" reflects n=1 in VMs, days, and machine-types; see §7 item 2 for the scaling and replication bounds.

**Production-readiness note (fleet headroom).** The 6-VM "minimum viable" sizing above has no failure-mode headroom: at steady state each VM runs ~85% sum CPU (two workers), so a single VM loss forces the remaining 5 VMs to absorb 20% more load — ~102% sum CPU before counting the activation-burst cost of re-claiming the evicted feeds. For production deployments we therefore recommend **N+1 or N+2 sizing (7 or 8 VMs)** at the k=2 configuration, revisable after a first-week production-traffic shakedown confirms between-VM variance. The 6-VM row is the measurement-derived minimum, not the production-provisioning target.
...
1. **Single run (no replication).** The ramp was executed once on one VM on one day; between-run variance (across VMs, regions, times of day, upstream Broadcastify traffic, cgroup noisy-neighbor conditions) is unbounded. A minimal replication (at least two additional ramps) would let us report mean ± SD for per-feed coefficients. This limitation subsumes time-of-day effects: the ramp ran 01:23–02:55 UTC (evening in the US), and radio feed activity and cloud-region noisy-neighbor variance differ across hours and days. **Population uncertainty is wider than the reported within-fit CI.** The 95% CI on the CPU slope (±0.0045 of 0.0689, or ±6.5%) is a within-fit measurement-error band, not a population-variance band; GCE noisy-neighbor variance alone can move per-core mean CPU by 5–15% on a single VM across 30-minute windows, which widens the defensible population band on the slope to approximately 0.060–0.085 %/feed before replication. Fleet-sizing decisions should use the wider band until replication is available.

2. **Multi-process validation bounded, not asymptotic.** Experiment 1c.A validates a two-container configuration on one VM (§6.4). This does not establish scaling to four or eight workers per VM, nor does it bracket architectures (gunicorn-style multi-worker-in-one-container vs multi-container-on-one-VM). A gunicorn pre-fork configuration could additionally reduce per-worker base RSS via copy-on-write page sharing of the parent interpreter (not possible with independent containers); the magnitude of this saving at our 7.15 MiB/feed marginal RSS cost is unmeasured but could meaningfully change the per-VM feed ceiling for memory-constrained configurations.
...
4. **No connection-pool instrumentation.** The GCS upload tail's activation-window character (§5.5) is *consistent with* pool-depth saturation during mass lease acquisition, but we did not instrument the GCS client's HTTP/2 connector pool depth; causal attribution is inferential, not directly verified. A targeted follow-up would tune `aiohttp.TCPConnector(limit=...)` (or the equivalent `ClientSession` knob for whichever HTTP client the GCS library uses) to values above the expected concurrent-upload count during activation, and re-measure the p95/p99 tail — if the bimodal distribution collapses to steady-state alone, the pool-saturation hypothesis is confirmed.
...
13. **No machine-type A/B.** All measurements are on `n2-standard-4`. The `e2` family uses a fractional vCPU model that can diverge from `n2` above ~25% sustained per-core utilization per Google documentation. The 77% target recommended in §6.1 could be silently penalized on `e2`. A matched ramp on `e2-standard-4` would bracket this. The compute-optimized `c2` family (and its `c2d` / `c3` successors) is a priori well-suited to single-thread-bound workloads because sustained per-core frequency headroom is higher than on `n2`; a matched `c2-standard-4` ramp would bound the upper end of single-worker density at the cost of per-hour VM price.
...
18. **Operational limitations not covered by the measurement paper.** The paper measures steady-state and activation-burst behavior on an intentionally-controlled catalog; it does not cover production-operational behavior that a fleet-sizing *deployment* would need to bound before committing to committed-use discounts. Specifically: (a) **single-VM-loss evacuation** — when one VM in a 6-VM k=2 fleet fails, 2,000 feeds must re-lease to the remaining 5 VMs, pushing their sum CPU from ~85% to ~102% while also absorbing an activation-burst cost; the §6.1 production-readiness note reflects this. (b) **AlloyDB failover** — all 12 workers simultaneously reconnect and re-lease, which is the subprocess-spawn-storm pattern at 6× the scale of the measured k=2 burst. (c) **Rolling-restart stampede** — the §5.4 mitigation "stagger activation" has no prescribed inter-worker stagger interval. (d) **Upstream outage recovery** — Broadcastify returning 500s for an extended period elevates ffmpeg crash rate and subprocess churn. (e) **Host-level SSH unresponsiveness during activation burst** — observed in 1c.A for ~6 minutes (load avg 3–5 on the 4-vCPU VM) during the simultaneous two-container 1,000-feed activation; while the ingestion pipeline itself continued uninterrupted (0 GCS failures, containers kept claiming feeds and uploading), operator incident-response visibility is degraded during that window. All five items are operational-readiness concerns that should be bounded by a production canary before full rollout.
...
From these coefficients we recommend **1,000 feeds per worker for steady state** and **~1,050 peak-tolerable**. For the 12,000-feed production target, this implies **12 workers**; multi-process scaling is **demonstrated at k=2 containers per VM in a single run** (§6.4, Experiment 1c.A: sum CPU 85.2% vs predicted 85.7%), yielding a measurement-minimum of **6 n2-standard-4 VMs under the k=2 configuration**. Production deployments should size to N+1 or N+2 (7 or 8 VMs) for failure-mode headroom; see §6.1 production-readiness note and §7 item 18. Scaling to k=4 or k=8 workers per VM is not measured and would require additional bracketing (§7 item 2). Per-source decomposition (§5.8) yields point-estimate slopes — bcfy_feeds 0.156 %/feed (ffmpeg subprocess per feed), openmhz 0.100 %/feed (high upload rate), bcfy_calls 0.009 %/feed (HTTP polling only) — with n=3 bootstrap CIs that cannot reject a zero-slope null at 95% confidence; the out-of-sample additive validation against the 41:55:4 mixed workload (predicted 78.8% CPU vs 77.4% observed, +1.8% residual) is suggestive of additivity but compatible with a ±5–10% between-day variance envelope and does not by itself confirm the additive model. Formal inference-grade CIs and same-day matched-composition additive validation are deferred to a Phase 2 campaign with ≥ 5 levels × 3 replicates per source.

The paper's contribution is the coefficients and their confidence intervals, the workload-mix-specific saturation point, the pre-flight validation methodology, and the fleet-sizing translation — not a rediscovery of asyncio's single-threaded property. Experiment 1c narrows three of the original gaps: multi-process scaling is demonstrated at k=2 (§6.4); per-source coefficients are measured as point estimates (§5.8); and the 9.7-s drift outlier is reproduced and attributed to concurrent mass `posix_spawn` subprocess creation (§5.4, with the in-process-monitor caveat that starvation vs blocking-syscall on the loop thread are not separable without off-CPU profiling). The strongest remaining gaps — multi-day replication, allocator bracketing, machine-type A/B, and k=4+ scaling — are enumerated in §7 with specific follow-up recipes.
