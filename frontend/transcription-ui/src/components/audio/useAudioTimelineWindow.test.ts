...
describe('useAudioTimelineWindow', () => {
  ...
  it('recenters the window on the playing segment when it leaves the window, marking it scrubbed', () => {
  ...
  it('recenters the window on the highlighted segment when it leaves the window', () => {
    const recent = seg('recent', '10:00:00', '10:00:05');
    const old = seg('old', '08:00:00', '08:00:05');
    const { result, rerender } = renderHook(
      (props: Parameters<typeof useAudioTimelineWindow>[0]) =>
