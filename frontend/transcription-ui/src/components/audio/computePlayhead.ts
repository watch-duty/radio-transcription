...
// Positions the single playhead marker for the given playback state: listening
// rests at the live edge; playing follows the clip; paused freezes there (or at
// the live edge if paused straight from listening).
export function computePlayhead({
  ...
  const playingSegment = currentlyPlayingSegmentId
    ? audioSegments.find((s) => s.id === currentlyPlayingSegmentId)
    : undefined;
