...
// Even with priming trimmed, a segment boundary is a hard cut between two independently
// sourced/encoded clips — their waveforms essentially never line up in amplitude, so the cut
// itself can still click (the same reason audio editors always cross-fade/declick a splice
// rather than relying on finding a "clean" cut sample). We mask it with a brief symmetric gain
// dip scheduled right at the splice via the existing GainNode.
const DECLICK_HALF_WIDTH_SECONDS = 0.008; // ~16ms total dip
