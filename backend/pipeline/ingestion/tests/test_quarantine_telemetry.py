...
class TestEmitQuarantineEvent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_emits_structured_log(self) -> None:
        ...
        record = cm.records[0]
        self.assertEqual(record.event_type, "feed_quarantined")
        self.assertEqual(record.feed_id, "abc-123")
        self.assertEqual(record.feed_name, "Test Feed")
        self.assertEqual(record.source_type, "bcfy_feeds")
