...
class _ImmediateExecutor:
    ...
    async def execute(
        ...
        return _types._ExecutorCompleted()
...
def _actual_model_projection(
    snapshot: _types._ShardSnapshot,
) -> _ModelSnapshot:
    return _ModelSnapshot(
        held=snapshot.held,
    ...
    async def test_seeded_shard_snapshot_matches_model_transitions(
        ...
        for seed in (0xB0A7, 0xAFF1):
            with self.subTest(seed=seed):
                ...
                model = _ReferenceModel()
                records: dict[int, _types._CallRecord] = {}
                source_order = 0
...
class TestShardWorkers(unittest.IsolatedAsyncioTestCase):
    ...
    def _limits(self, *, workers: int = 2) -> _types._SchedulerLimits:
    ...
    async def test_swallowed_cancellation_fails_closed_and_retains_token(
        ...
        limits = _types._SchedulerLimits(
            shard_count=1,
            capacity=2,
            workers_per_shard=1,
            high_water=1,
