...
def generate_transmission_id(feed_id: str, start_ms: int) -> str:
    """Creates a deterministic UUID string using uuid5 to ensure pipeline retries produce the exact same ID.

    Uses raw VAD start time to ensure stability across pre-roll/post-roll configuration changes.
    """
    deterministic_id = f"{feed_id}_{start_ms}"
