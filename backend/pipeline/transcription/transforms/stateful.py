...
def _get_task_logger(
    ...
    return std_logging.LoggerAdapter(
        trans_logging.get_logger(__name__),
        {
...
@beam.typehints.with_output_types(tuple[str, datatypes.ChunkMetadata])
class ParseAndKeyFn(beam.DoFn):
    """Unmarshalls Pub/Sub JSON payloads into ChunkMetadata.

    Outputs valid elements on main tag, and unparseable inputs on DLQ.
    """

    def __init__(self) -> None:
        self.parsed_counter = Metrics.counter(self.__class__, "parsed_messages")
        self.error_counter = Metrics.counter(
            self.__class__, "unparseable_payloads"
        )
    ...
    def process(
        ...
        tuple[str, datatypes.ChunkMetadata]
        ...
        def _parse_payload(
            ...
            with tracing_utils.with_tracer_context(
                traceparent, "parse_pubsub_payload", __name__
            ):
                chunk_dict = payload.get("audio_chunk", {})
                if not chunk_dict:
                    msg = "Pub/Sub payload is missing 'audio_chunk' block."
                    raise ValueError(msg)

                feed_id = chunk_dict.get("feed_id")
                if not feed_id:
                    msg = "Pub/Sub payload is missing 'feed_id' key."
                    raise ValueError(msg)

                # 2. Extract chunk metadata
                feed_metadata_dict = chunk_dict.get("feed_metadata", {})
                feed_metadata = datatypes.FeedMetadata(
                    feed_name=feed_metadata_dict.get("feed_name", ""),
                    external_id=feed_metadata_dict.get("external_id", ""),
                )
                metadata = datatypes.ChunkMetadata(
                    gcs_uri=chunk_dict.get("gcs_uri", ""),
                    session_id=chunk_dict.get("session_id", ""),
                    duration_ms=chunk_dict.get("duration_ms", 0),
                    feed_metadata=feed_metadata,
                    traceparent=traceparent,
                )
                return feed_id, metadata

        try:
            # 1. Parse Pub/Sub raw bytes
            feed_id, metadata = _parse_payload(element)
            self.parsed_counter.inc()
            yield (feed_id, metadata)

        except Exception as e:
            self.error_counter.inc()
            logger.exception("Unparseable Pub/Sub payload.")
            dlq_payload = {
                "feed_id": "unknown",
                "gcs_uri": "unknown",
                "session_id": "unknown",
                "error_message": f"Unparseable: {e}",
                "traceparent": traceparent,
            }
            yield beam.pvalue.TaggedOutput("transcription_dlq", dlq_payload)
...
@beam.typehints.with_output_types(datatypes.TranscriptionResult)
class TranscribeAudioFn(beam.DoFn):
    ...
    def __init__(
        self,
        config: datatypes.TranscribeAudioConfig,
        transcriber_factory: Any | None = None,
    ) -> None:
        self.config = config
        self.transcriber_factory = (
            transcriber_factory or transcribers.get_transcriber
        )
        ...
        self.speech_duration_sec_dist = Metrics.distribution(
            self.__class__, "speech_duration_sec"
        )
        self.transcription_time_ms = Metrics.distribution(
            self.__class__, "transcription_time_ms"
        )

        self.audio_processor: audio_processor.AudioProcessor | None = None
        self.transcriber: transcribers.Transcriber | None = None
        self.audio_uploader: gcs_uploader.GCSAudioUploader | None = None
    ...
    def _export_and_transcribe(
        ...
        """Concatenates buffered chunks, renders to an intermediate audio format, and dispatches to the ASR client."""
        ...
        if not self.config.canonical_audio_bucket:
            ...
            playback_audio_uri = (
                f"gs://{self.config.canonical_audio_bucket}/{m4a_path}"
            )
        ...
        return datatypes.TranscriptionResult(
            ...
            or "gs://test-bucket/placeholder.flac",
