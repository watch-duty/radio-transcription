...
class ParseAndKeyFn(beam.DoFn):
    ...
    def process(
        ...
        def _validate(proto: AudioChunk) -> None:
            if not proto.feed_id:
                msg = "AudioChunk missing required feed_id"
                raise ValueError(msg)
            if not proto.gcs_uri:
                msg = "AudioChunk missing required gcs_uri"
                raise ValueError(msg)
            if not proto.session_id:
                msg = "AudioChunk missing required session_id"
                raise ValueError(msg)
            if proto.duration_ms <= 0:
                msg = "AudioChunk missing or invalid duration_ms"
                raise ValueError(msg)

        try:
            chunk_proto = AudioChunk()
            chunk_proto.ParseFromString(element.data)
            _validate(chunk_proto)
            ...
            msg = f"Failed to parse payload or missing feed_id: {e}"
