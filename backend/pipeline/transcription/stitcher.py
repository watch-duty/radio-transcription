...
class StitchAudioFn(beam.DoFn):
    ...
    def _apply_flush_action(
        ...
        buffered_audio = action.isolated_audio_buffer or list(transmission_buffer.read())
        if buffered_audio:
            yield FlushRequest(
            ...
            logger.warning(f"FlushAction emitted but BagState was empty for feed {action.feed_id}.")
