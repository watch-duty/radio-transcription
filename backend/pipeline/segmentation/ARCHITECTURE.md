...
The `DEFAULT_PUBSUB_FALLBACK_DRAIN_TIMEOUT_MS` default is deliberately conservative, not measured. Its floor is peak system lag — a timeout short enough to fall inside that band force-drains segments the backlog merely delayed, rather than lost ones — and the lag figures setting that floor live in the constant's comment, next to the value they justify. `bcfy_feeds` produces roughly 150-400 sessions/day, so `pubsub_order_gap_resolution_seconds` accumulates slowly: **read its maximum and shape before trusting a percentile.** Tighten only once the distribution is populated, and expect skew to be worst during autoscaling events, when cold workers join the pool.
