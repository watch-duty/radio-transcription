...
GCS_PATH_TEMPLATE = "{feed_id}/{year}/{month}/{day}/{segment_id}"
LOSSLESS_PREFIX = "lossless"
...
class NormalizationEventProcessor:
    ...
    def process_event(self, cloud_event: CloudEvent) -> None:
        ...
        with with_tracer_context(
            ...
            try:
                ...
                base_key = GCS_PATH_TEMPLATE.format(
                    feed_id=feed_id,
                    year=f"{dt:%Y}",
                    month=f"{dt:%m}",
                    day=f"{dt:%d}",
                ...
                flac_path = f"{LOSSLESS_PREFIX}/{base_key}.flac"
                m4a_path = f"{PLAYBACK_PREFIX}/{base_key}.m4a"
                ...
                if _is_transient_exception(e):
                    logger.warning(
                        "Transient failure processing segmented audio claim for segment %s (feed %s): %s. Retrying...",
                        segment_id,
                        feed_id,
                        e,
                    )
                ...
                logger.exception(
                    "Permanent failure processing segmented audio claim for segment %s (feed %s): %s. Acknowledging message without retry.",
                    segment_id,
                    feed_id,
                    e,
                )
    ...
    def _normalize_buffer(
        ...
        """Normalizes the audio buffer to FLAC."""
