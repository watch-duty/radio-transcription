...
class OrderedContinuousStitchAudioFn(beam.DoFn):
    ...
    def process(
        ...
        tuple[str, datatypes.FlushRequest] | datatypes.TranscriptionDlqOutput
    ]:
...
@beam.typehints.with_output_types(datatypes.NormalizationResult)
class NormalizeAudioFn(beam.DoFn):
    """Normalizes consolidated audio buffers by applying acoustic filtering,
    localized Voice Activity Detection (VAD) mapping, silence suppression, and GCS uploading.
    Outputs a lightweight NormalizationResult claim-check payload.
    """

    def __init__(
        self,
        config: datatypes.NormalizeAudioConfig,
    ) -> None:
        self.config = config
        ...
        self.normalization_count = Metrics.counter(
            self.__class__, "normalization_count"
        )
    ...
    def _export_and_normalize(
    ...
    ) -> datatypes.NormalizationResult | None:
        """Concatenates buffered chunks, renders to FLAC/M4A, and uploads them to GCS."""
        ...
        return datatypes.NormalizationResult(
    ...
    def process(
        ...
        **kwargs: Any,
        ...
        datatypes.NormalizationResult | datatypes.TranscriptionDlqOutput
    ]:
        ...
        outputs = []
        with tracing_utils.with_tracer_context(
            request.traceparent or "", "normalize_audio", __name__
        ):
            try:
                normalized = self._export_and_normalize(request)
                if normalized:
                    self.normalization_count.inc()
                ...
                if is_validation:
                    logger.warning(
                        f"Validation failure during normalization for feed {feed_id}: {e}"
                    )
                else:
                    logger.exception(
                        "Unexpected error normalizing buffer for feed %s",
                        feed_id,
                    )
