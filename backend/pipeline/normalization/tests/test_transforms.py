...
class OrderedContinuousStitchSpeechSegmentsTest(unittest.TestCase):
    ...
    def test_stale_traceparent_discarded_on_new_transmission(
        self, mock_audio_processor: MagicMock
    ) -> None:
        """Verifies that a stale traceparent from a previous transmission is discarded
        and replaced by the incoming chunk's traceparent when starting a new transmission.
        """
        mock_processor_inst = mock_audio_processor.return_value
        chunk_data = AudioChunkData(
            start_ms=100000,
        ...
        # Mock active state that has completed/flushed previous transmission
        # So stale_start_time_ms is None, but traceparent still has a stale traceparent
        mock_state_context = MockValueState(
            ActiveStitchingState(
                session_id="session-1",
                stale_start_time_ms=None,
        ...
        # Assert that the traceparent in the saved context was updated to the fresh one!
        saved_context = mock_state_context.read()
        self.assertIsInstance(saved_context, ActiveStitchingState)
        self.assertEqual(saved_context.traceparent, "fresh-traceparent")
