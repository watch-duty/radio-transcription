...
class NormalizeAudioTest(unittest.TestCase):
    ...
    def test_normalize_micro_segment_skipped(
        self, mock_audio_processor: MagicMock
    ) -> None:
        """Verifies that when processed audio is under the 200ms threshold,
        NormalizeAudioFn cleanly drops the transmission without throwing errors.
        """
        mock_processor_inst = mock_audio_processor.return_value
    ...
    def test_serialize_claim(self) -> None:
        ...
        with BeamTestPipeline(options=options) as p:
            ...
            def assert_results(msgs):
                from backend.pipeline.schema_types.audio_ready_for_transcription_pb2 import (  # noqa: PLC0415
