...
class MonitoringClient:
    ...
    async def write_time_series_double(
        self,
        metric_type: str,
        labels: dict[str, str],
        value: float,
        *,
        ...
        """Write a single GAUGE DOUBLE data point to Cloud Monitoring.

        Mirrors ``write_time_series`` but for ``DOUBLE`` value-type custom
        metrics (e.g. ``oldest_unclaimed_age_seconds`` whose descriptor
        declares ``value_type = "DOUBLE"``, ``unit = "s"``). Required
        because the GCP autoscaler signal must be DOUBLE; INT64 is silently
        incompatible at the descriptor level.

        Args:
            metric_type: e.g.
                ``custom.googleapis.com/feeds/oldest_unclaimed_age_seconds``.
            labels: metric labels (callers enforce allowlist).
            value: float64 GAUGE value. Pass exactly the value to write
                (including 0.0 — no truthy-check skipping).
            resource_type: monitored-resource type. Defaults to ``"global"``.
            resource_labels: resource labels (required). Caller owns the
                full label dict.
        """
        series = monitoring_v3.TimeSeries()
        series.metric.type = metric_type
