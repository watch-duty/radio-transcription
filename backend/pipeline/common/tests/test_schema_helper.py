...
class TestSchemaHelper(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_async_apply_test_schema(self, mock_sql_dir: MagicMock) -> None:
        ...
        mock_sql_dir.glob.return_value = [mock_file_normal, mock_file_autocommit]
        ...
        mock_conn.execute.assert_any_call("-- AUTOCOMMIT\nALTER TYPE a ADD VALUE 'B'")
    ...
    def test_sync_apply_test_schema(self, mock_sql_dir: MagicMock) -> None:
        ...
        mock_conn.execute.assert_any_call("CREATE TABLE a (id UUID);".encode())
        mock_conn.execute.assert_any_call("-- AUTOCOMMIT\nALTER TYPE a ADD VALUE 'B'".encode())
        mock_conn.execute.assert_any_call("ALTER TYPE a ADD VALUE 'C'".encode())
