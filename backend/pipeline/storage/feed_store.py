...
leased AS (
    ...
    RETURNING feeds.id, feeds.name, feeds.source_type,
              feeds.last_processed_filename, feeds.last_bookmark,
              feeds.fencing_token
)
SELECT leased.id, leased.name, leased.source_type,
       leased.last_processed_filename, leased.last_bookmark,
...
class FeedStore:
    ...
    async def lease_feed(self, worker_id: uuid.UUID) -> LeasedFeed | None:
        ...
        return LeasedFeed(
            ...
            last_bookmark=row["last_bookmark"],
    ...
    async def update_feed_progress(
        ...
        Args:
            ...
            last_bookmark: Timestamp bookmark for the last processed audio.
