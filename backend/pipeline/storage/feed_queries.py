...
WITH new_feed AS (
    ...
    VALUES ($1, $2)
    RETURNING id, name, source_type, status, failure_count, created_at
),
new_props AS (
    INSERT INTO feed_properties (feed_id, source_feed_id, external_id)
    SELECT id, $3, $4 FROM new_feed
    WHERE $3 IS NOT NULL AND $4 IS NOT NULL
)
SELECT nf.*, fp.source_feed_id, fp.external_id
FROM new_feed nf
LEFT JOIN feed_properties fp ON nf.id = fp.feed_id
"""

GET_FEED_SQL = """\
SELECT f.*, fp.source_feed_id, fp.external_id
FROM feeds f
